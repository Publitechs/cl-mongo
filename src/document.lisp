...
(defun set-field-value (document slot new-value)
  (setf (gethash (slot-definition-name slot) (elements document)) new-value))

(defun get-field-value (document slot &optional default)
  (gethash (slot-definition-name slot) (elements document) default))
...
(defun unbound-field (document slot)
  (remhash (slot-definition-name slot) (elements document))
  (mark-as-changed))
...
(defclass document ()
  ((elements  :initarg :elements :accessor elements :printable nil)
...
(defun ht->document.1 (ht) 
  (multiple-value-bind (oid oid-supplied) (gethash "_id" ht)
	  ...
	  (progn 
	    (remhash "_id" ht)
	    (make-instance 'document :_id oid :local nil :elements ht))
