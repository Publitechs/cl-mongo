...
(defun mark-as-changed (document)
  (setf (slot-value document '_saved) nil))
...
(defclass document ()
  ((_elements  :initarg :elements :reader _elements :printable nil)
   ...
   (_saved :initarg :saved :reader saved :printable nil))
  (:default-initargs
   :local t
   :saved nil
   :_id (make-bson-oid))
...
(defmethod set-saved ((document document))
  (setf (slot-value document '_saved) t))
...
(defun build (class ht)
  (let ((doc (ht->document.1 ht class))
        (class (find-class class)))
    (loop for slot-def in (class-slots class)
          do 
             (let* ((allocation (slot-definition-allocation slot-def))
                    (name (slot-definition-name slot-def))
                    (field-name (to-field-name name)))
               (when (and 
                       (eq :instance allocation)
                       (not (alexandria:starts-with-subseq "_" field-name)))
                 (setf (slot-value doc name) (gethash field-name ht))
                 (remhash field-name ht))))
    doc))
